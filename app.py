...
try:
    # UPDATED: Pointing to the '_new' files generated by the updated forecasting script
    ...
    print(f"CRITICAL ERROR: Could not load models. Ensure files end in '_new.joblib' and are in 'models/' folder. {e}")
    sys.exit(1)
...
def predict_dew_yield(day_df):
    """
    Calculates the 5 features expected by the NEW Dew MLP and predicts.
    Logic updated to match new model training parameters.
    """
    try:
        # Feature 1: Max RH
        ...
        # Predict (Returns mL/m2)
        pred_mL = dew_model.predict(features_scaled)[0]
        
        # Convert to L (divide by 1000)
        return max(0.0, float(pred_mL) / 1000.0)
    except Exception as e:
        print(f"Dew Prediction Error: {e}")
        return 0.0
...
def get_historic():
    ...
    daily_data = []
    fog_events = 0
    # Removed aggregate totals to ensure models are shown separately
    ...
    for day in unique_days:
        ...
        # NOTE: Total summation logic removed to focus on separate models
...
def run_simulation():
    ...
    for i in range(365):
        # Simulation logic placeholder (can be improved with real historic data)
